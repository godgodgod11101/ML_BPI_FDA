...
資料不平衡處理，使用 themis套件 搭配 recipes套件
```{r}
#| message: false
#| warning: false
...
先嘗試少數類別過採樣的方式（不合成新樣本），統一將少數類別樣本占整體比例調至30%
```{r}
...
# 檢查處理後占比
bind_rows(
    fail_ratio(train_upsample$all$Y), 
    fail_ratio(train_upsample$`130101`$Y), 
    fail_ratio(train_upsample$`140703`$Y)
)
...
```{r}
lst_mdlData[["train_upsample"]] <- train_upsample
# save(lst_mdlData, file = "data/vegetable_imb.rda")
```
