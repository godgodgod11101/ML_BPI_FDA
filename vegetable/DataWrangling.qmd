...
2. **Tidy & Transform**：資料清理與轉換，以方便使用 [tidymodels]{.pkg} 建模。
3. **Split**：切分訓練集與測試集，存成 [.rda]{.var} ，供後續建模使用。
...
將非用於建模之報單額外資訊另存於 [tbl_others]{.var} 資料表
```{r}
tbl_others <- tbl_ori %>% 
...
1. 目標變數轉換成 factor 型別，同時去掉中文類別名稱
2. 預測目標放第一個位置，配合 [tidymodels]{.pkg}
```{r}
...
1. 特徵轉換成 factor 型別（BPI模型資料皆為類別變數），同時去掉中文類別名稱
2. 除去「無...」類別（如無相關資料）
